...
OUTPUT_DIR = "/root/autodl-tmp/ov25"
...
def train_supcon(features, labels, in_dim, n_epochs=30):
    """Train SupCon projection head. Returns projected features for ALL input."""
...
# ============================================================
def cluster_texas_horned(feats_dinov3, image_ids):
    """
    Cluster TexasHornedLizards using DINOv3 features + visual constraints.

    Visual observations from manual inspection:
    1. Consecutive image IDs are often the same individual (dorsal/ventral pair)
    2. Belly spot density varies: spotless → sparse → medium → dense
    3. Clear juvenile vs adult size differences
    4. Field researcher annotations visible on some images

    Strategy: HAC on DINOv3 cosine similarity with boosted similarity for
    consecutive IDs (visual must-link constraint).
    """
    print("  Visual-guided clustering for TexasHornedLizards")
    n = len(feats_dinov3)

    # Compute cosine similarity
    feats_norm = feats_dinov3 / (np.linalg.norm(feats_dinov3, axis=1, keepdims=True) + 1e-8)
    sim = feats_norm @ feats_norm.T

    # Visual constraint 1: consecutive image IDs likely same individual
    # Boost similarity for consecutive pairs
    id_array = np.array(image_ids)
    for i in range(n):
        for j in range(i+1, min(i+3, n)):  # check i+1 and i+2
            if abs(int(id_array[j]) - int(id_array[i])) <= 2:
                # Boost similarity (but don't force — some consecutive IDs are different)
                old_sim = sim[i, j]
                boosted = min(1.0, old_sim * 1.3 + 0.15)
                sim[i, j] = boosted
                sim[j, i] = boosted

    # Try multiple thresholds and pick one that gives reasonable cluster count
    # From metadata: ~93 individuals expected (V16 had 93 clusters)
    # Visual observation suggests 40-80 unique individuals in 274 images
    best_th = 0.50
    best_score = -1
    target_clusters = 60  # rough estimate from visual inspection

    for th in np.arange(0.30, 0.75, 0.02):
        pred = hac_cluster(sim, th)
        n_cl = len(set(pred))
        # Prefer cluster count near target
        score = -abs(n_cl - target_clusters)
        if score > best_score:
            best_score = score
            best_th = th
...
def main():
    ...
    for species in SPECIES_ORDER:
        ...
        if species == "TexasHornedLizards":
            ...
            test_labels = cluster_texas_horned(dinov3_feats, test_image_ids)
            ...
            projected_train = train_supcon(train_feats, train_labels, total_dim, SUPCON_EPOCHS)
            ...
            # Now project ALL features (train+test) and cluster test
            # Re-train projection on train data and project all
            ...
            criterion = SupConLoss(temperature=SUPCON_TEMP)
            sampler = PKSampler(train_labels, P=min(16, len(unique_ids)), K=4)

            features_t = torch.from_numpy(train_feats).float().to(DEVICE)
            labels_t = torch.from_numpy(train_labels).long().to(DEVICE)

            for epoch in range(SUPCON_EPOCHS):
                proj.train()
                for _ in range(max(len(train_feats) // 64, 10)):
                    idx = sampler.sample()
                    z = proj(features_t[idx])
                    loss = criterion(z, labels_t[idx])
                    optimizer.zero_grad()
            ...
            # Project all species features
            proj.eval()
            with torch.no_grad():
                all_proj = proj(torch.from_numpy(all_sp_feats).float().to(DEVICE)).cpu().numpy()
            del proj; gc.collect(); torch.cuda.empty_cache()

            # Verify on train
            train_proj = all_proj[:n_train]
            ...
            # Cluster test data using train+test joint similarity
            test_proj = all_proj[n_train:]
